...
def select_ids(fasta, out, selected):
    x = open(out, "a")    
    for record in SeqIO.parse(fasta, "fasta"):
        recordid = record.id
        recordseq = record.seq
        if recordid in selected:
            x.write(">"+recordid)
            x.write("\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-select", metavar="", dest="select",
                        help="Specify list of selected ids")
    ...
    if args.inputFASTA is None and args.inputOUT is None:
        ...
        select_ids(args.inputFASTA, args.inputOUT, selected)